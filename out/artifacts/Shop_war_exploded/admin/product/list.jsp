		...
		<ul class="pagination" style="text-align: center; margin-top: 10px;">
			...
			<c:forEach begin="1" end="${totalPage}" var="pageNum">
			   <c:if test="${currentPage==pageNum}">
			      <li class="active"><a href="ProductListServlet?currentPage=${pageNum}&admin=admin">${pageNum }</a></li>
			   </c:if>
			   <c:if test="${currentPage!=pageNum }">
			      <li><a href="ProductListServlet?currentPage=${pageNum}&admin=admin">${pageNum}</a></li>
			   </c:if>
			</c:forEach>
			
			<c:if test="${currentPage!=totalPage}">
			    <li><a href="ProductListServlet?currentPage=${currentPage+1}&admin=admin" aria-label="Next"> <span aria-hidden="true">&raquo;</span>
			         </a>
			    </li>
			</c:if>
